...
export const BatchAnalytics: React.FC<BatchAnalyticsProps> = ({ orders, batchCosts, onUpdateBatchCost, initialEditBatch }) => {
  ...
  const batchData = useMemo(() => {
    ...
    return Object.entries(batches).map(([name, batchOrders]) => {
      ...
      return {
        batchName: name,
        orderCount,
        _internalTotalItems: totalItems,
        totalSales,
        deliveryFee,
        oatPayment,
        totalCostPrice,
  ...
  const monthlyData = useMemo(() => {
    ...
    batchData.forEach(batch => {
      ...
      m._internalTotalItems += batch._internalTotalItems;
  ...
  useEffect(() => {
    if (initialEditBatch) {
      ...
      if (batch) {
        ...
        const initialDeliveryQty = storedCost?.deliveryFeeQuantity !== undefined ? storedCost.deliveryFeeQuantity : batch._internalTotalItems;
