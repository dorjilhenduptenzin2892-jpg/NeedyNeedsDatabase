...
const isConfigured = () => {
  return WEB_APP_URL && WEB_APP_URL.startsWith("https://script.google.com") && !WEB_APP_URL.includes("YOUR_PASTED");
};
...
export const syncOrdersToSheet = async (orders: Order[]) => {
  ...
  try {
    // We use no-cors for fast async sync, but the bridge code fix (sheet.clear) 
    // ensures the logic on the server side is sound.
...
export const syncBatchCostsToSheet = async (costs: BatchCost[]) => {
  ...
  try {
    ...
    console.error("Sync Costs failed:", err);
  }
};
